	...
	var ui_class = {
		...
		updateHostname: function()
		{
			return L.system.getBoardInfo().then(function(info) {
                ...
                /*zoe add for network restart*/
                networkRestart: L.rpc.declare({
                object: 'network',
                method: 'restart',
                expect: { }
                }),
			...
			return L.uci.changes().then(function(changes) {
				...
				if (n > 0)
					$('#changes')
							...
							L.ui.dialog(L.tr('Staged configuration changes'), html, {
								...
								confirm: function() {
									L.uci.apply().then(
										...
										L.ui.networkRestart();    /*zoe add*/
									    },
									    function(code) { alert('Error with code ' + code); }
									);
									L.ui.dialog(false);               /*zoe add*/		
								}
							});
							ev.preventDefault();
						})
						.children('span')
							.show()
							.text(L.trcp('Pending configuration changes', '1 change', ' Apply '));  /*zoe add*/ 
				else
