...
Bool SW_XXXX_Upgrade(Parameters_t *param) {
    ...
    if ((p == i386) || ((p != unknown) && (p != mips))) {
        sprintf(cmd, "fwdldhosti686 -s /usr/sbin/slqssdk -d 9x15 -p %s 2>&1", (char *) param->directory);
    } else if (p == mips) {
        sprintf(cmd, "fwdldmipsel -s /usr/sbin/slqssdk -d 9x15 -p %s 2>&1", (char *) param->directory);
    } else {
        return false;
    }
