...
kaa_error_t kaa_client_create(kaa_client_t **kaa_client, kaa_client_props_t *props)
{
    ...
    if (error_code) {
        KAA_LOG_ERROR(self->kaa_context->logger, error_code, "Failed to init Kaa log collector, error %d", error_code);
        kaa_client_destroy(self);
        return error_code;
    }
#endif
...
kaa_error_t kaa_client_process_channel_connected(kaa_client_t *kaa_client)
{
    ...
    kaa_tcp_channel_get_descriptor(&kaa_client->channel, &channel_fd);
    ...
    if (poll_result == 0) {
        ...
        KAA_LOG_ERROR(kaa_client->kaa_context->logger, KAA_ERR_BAD_STATE, "Failed to poll descriptors: %s", strerror(errno));
...
kaa_error_t kaa_client_process_channel_disconnected(kaa_client_t *kaa_client)
{
    ...
    if (error_code) {
        KAA_LOG_ERROR(kaa_client->kaa_context->logger, error_code, "Failed to connect channel [0x%08X]", kaa_client->channel_id);
    } else {
        KAA_LOG_TRACE(kaa_client->kaa_context->logger, error_code, "Channel [0x%08X] successfully connected", kaa_client->channel_id);
...
kaa_error_t kaa_client_deinit_channel(kaa_client_t *kaa_client)
{
    ...
    error_code = kaa_channel_manager_remove_transport_channel(kaa_client->kaa_context->channel_manager,kaa_client->channel_id);
    if (error_code) {
    ...
    KAA_LOG_TRACE(kaa_client->kaa_context->logger, KAA_ERR_NONE, "Channel deinitialized successfully");
...
kaa_error_t kaa_log_collector_init(kaa_client_t *kaa_client)
{
    ...
    KAA_LOG_INFO(kaa_client->kaa_context->logger, KAA_ERR_NONE, "Log collector init completed");
    return error_code;
}
#endif
